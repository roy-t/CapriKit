...
/// invalidates a live asset and never leaves a lease behind in the <see cref="AssetCache"/>.
...
/// </summary>
internal sealed partial class HotReloadManagerV3 : IDisposable
{
    private readonly ILogger<HotReloadManagerV3> Logger;
    ...
    /// step and read half-written files.
    /// </summary>
    public HotReloadManagerV3(ILoggerFactory loggerFactory, AssetCache cache, ScopedFileSystem fileSystem, TimeSpan? debounce = null)
    {
        Logger = loggerFactory.CreateLogger<HotReloadManagerV3>();
        Cache = cache;
    ...
    // The three methods below must be called while holding the lock.
